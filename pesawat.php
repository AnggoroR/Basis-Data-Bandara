...
$pesawat = query("SELECT * FROM pesawat");

// Jika tombol cari ditekan
if (isset($_POST["cari"])) {
    $pesawat = cari($_POST["keyword"]);
}
        ...
        <table class="table table-borderless table table-striped table-light">
            ...
            <?php foreach($pesawat as $row) : ?>
                <tr>
                    ...
                    <td><?= $row["harga"]; ?></td>
                    <td>
                        <a href="ubah (pesawat).php?kd_pesawat=<?= $row["kd_pesawat"]; ?>">
                            <button type="button" class="btn btn-success btn-sm">Ubah</button>
                        </a>
                        <a href="hapus (pesawat).php?kd_pesawat=<?= $row["kd_pesawat"]; ?>" onclick="return confirm('Apakah kamu yakin ingin menghapus data ini?');">
