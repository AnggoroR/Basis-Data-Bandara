...
<header>
    ...
    <div class="container">
        <div class="d-flex justify-content-md-center flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3">
            <h2 class="h2">Data Tiket Penerbangan</h2>
        </div>
        <table class="table table-borderless table table-striped table-light">
            ...
            <?php foreach($tiket as $row) : ?>
                <tr>
                    ...
                    <td><?= $row["waktu"]; ?></td>
                    <td>
                        <a href="ubah (tiket).php?no_tiket=<?= $row["no_tiket"]; ?>">
                            <button type="button" class="btn btn-success btn-sm">Ubah</button>
                        </a>
                        <a href="hapus (tiket).php?no_tiket=<?= $row["no_tiket"]; ?>" onclick="return confirm('Apakah kamu yakin ingin menghapus data ini?');">
